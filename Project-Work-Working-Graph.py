...
def Frame3():
    ...
    canvas.grid(row=1,column=1, padx=10, pady=10, columnspan=5, rowspan=10)
    #Image
    #img = Image.open('Title2.gif')
    #tkimage = ImageTk.PhotoImage(im) 
    #Tkinter.Label( image=tkimage).grid(row=1, column=1, padx=10, pady =5) #Image location
    ##Input for graph
    GenterA = Entry(frame3)
    GenterA.insert(0, "Insert A here")#Input boxes make look pretier  
    GenterA.grid(row=4, column=6)
    GenterB = Entry(frame3) #Input boxes make look pretier
    GenterB.insert(0, "Insert B here")
    GenterB.grid(row=4, column=7)
    GenterC = Entry(frame3) #Input boxes make look pretier
    GenterC.insert(0, "Insert C here")
    GenterC.grid(row=4, column=8)
    SCALER = Scale(frame3, from_=1, to=100)
    SCALER.grid(row=5, column=6)
    Scale_Label = Label(frame3, text="Scale Slider")
    Scale_Label.grid(row=6, column=6)
    X1_Label = Label(frame3, text="X1=")
    X1_Label.grid(row=7, column=7)
    X2_Label = Label(frame3, text="X2=")
    X2_Label.grid(row=8, column=7)
    def Clear():
        canvas.delete("all")
    
    #Button for convertor
    buttonSim = Tkinter.Button(frame3, text="Equation Convertor", command=FrameSIM)
    buttonSim.grid(row=12, column=1)
    buttonUse = Tkinter.Button(frame3, text="How to use", command=Frame1)
    buttonUse.grid(row=12, column=2)
    buttonEx = Tkinter.Button(frame3, text="Examples", command=Frame2)
    buttonEx.grid(row=12, column=3)
    #######
    buttonReset = Tkinter.Button(frame3, text="Reset Graph", command=Clear)
    buttonReset.grid(row=5, column=8)
    #Graph Function
    def length(i):   #Quick define Length
      return len(str(i))
    def GraphButton():
    ...
    buttonSim.grid(row=5, column=7)
        #Output box
    #w = Text(frame3, x1)
    #w.grid(row=7, column=8)
    #X1out = Entry(frame3)
   # X1out.insert(0, x1)#Input boxes make look pretier  
    #X1out.grid(row=7, column=8)
    #X2out = Entry(frame3)
    #X2out.insert(0, Gloobal)#Input boxes make look pretier  
    #X2out.grid(row=8, column=8)
